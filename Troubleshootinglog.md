...
### T-0718-1220-main · ② 브리핑 live 응답에서 사실 카드 미표시 — 사실 0건 강등 부재 + 감사로그 관측 공백
**발생 상황** — W2 완료 직후 사용자 리허설(12:15, `real_005930`·live)에서 ② 화면에 해석·알 수 없음·다음 질문만 표시되고 사실 카드가 비어 있음.
**증상** — `b.facts`가 빈 배열로 도착하면 renderStep2가 사실 섹션만 조용히 비움(해석·모름은 정상 렌더 — fact 단위 차단 설계의 의도된 동작이 UI 설명 없이 노출됨). 감사로그에는 원천·시도만 기록되어 차단/누락 여부를 사후 확인할 수 없었음.
**확인된 원인** — live LLM 응답의 비결정성: 12:15 응답 1회가 facts 0건으로 도착(모델이 비웠거나 guard가 전건 차단 — 당시 감사로그 미기록으로 어느 쪽인지 확정 불가). 배제 검증: 백엔드 static 경로 facts 5건 정상, W2 프런트 변경은 전부 cmp- 스코프로 renderStep2 무접촉, W4 "(가상)" 라벨 분기는 예외 시 해석·모름도 죽는 구조라 불일치. 동일 조건 live 재현 1회는 정상(facts 5·차단 0) — 간헐 사건 확정.
**조치(최종 해결)** — 계약 §8에 "사실 0건 강등" 명문화 후 브랜치 `wt/briefing-facts-fallback`에서: ① `_build_briefing`에 강등 로직(live→cache→static, 실제 차단은 안전 카운터에 정직 반영, attempts에 `facts_empty_degraded(원천)` 기록) ② 감사로그에 facts_total·facts_rendered·guard_blocked·guard_counters 병기(관측 공백 해소) ③ renderStep2 안전망 상태 카드(도달 불가 경로 방어). 테스트 4건 신설(모델 비움/전건 차단/감사 기록/정상 경로) — 전체 361 통과·게이트 통과(`gate_20260718_1230`).
**재발 방지** — 비결정 원천(live)의 계약 위반 응답은 가드 차단만이 아니라 **폴백 계층이 흡수**한다는 원칙을 §8에 명문화. 감사로그에 렌더 관측치(사실 수·차단 수)를 항상 남겨 사후 진단 가능 상태 유지.
