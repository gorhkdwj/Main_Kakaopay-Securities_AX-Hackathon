r"""판단 여권 · S4 웹 UI 서버(FastAPI) — AI(LLM) 없이 완주되는 데모 본체.

실행 방법(반드시 프로젝트 루트에서 — src는 namespace package라 루트 실행 전제):

    .\.venv\Scripts\python.exe -m uvicorn src.webapp.app:app --port 8765
...
아키텍처 위치(구현 계획 §0 불변 원칙):
  ...
  ③ LLM·외부 네트워크 호출 코드가 없다(오프라인 fixture 완주 — 폴백 계층 3).

API 명세(입출력 상세는 각 핸들러 docstring):
  GET  /api/scenarios        시나리오 목록
  GET  /api/scenario/{id}    fixture 로드 → 정적 브리핑 조립 → guard → 화면 데이터
...
S5 인터페이스(LLM 결합 지점): compose_briefing()이 만드는 계약 §6 응답 dict를
LLM 출력(JSON)으로 교체하면 된다 — check_response 관문·카운터 집계·화면 렌더는
동일 경로를 그대로 쓴다.
"""
...
def create_app(fixtures_dir: "Path | str | None" = None,
               records_dir: "Path | str | None" = None) -> FastAPI:
    """S4 웹 UI FastAPI 앱을 만든다.
    ...
    """
    app = FastAPI(title="판단 여권 · S4 데모", docs_url=None, redoc_url=None,
    ...
    def api_scenario(scenario_id: str):
        """fixture 로드 → 정적 브리핑 조립 → guard 통과 → 화면 데이터 반환.
        ...
        response, aux = compose_briefing(fx)
        sanitized, record = check_response(response, None)
    ...
    def api_preview(payload: dict = Body(...)):
        ...
        # 수량 대조를 통과시킨다. S5에서 이 stub 자리가 LLM 출력으로 바뀐다.
        stub = {
            "facts": [],
            "interpretations": [],
            "unknowns": [],
        ...
        sanitized, record = check_response(stub, calculation=preview)
        accumulate_guard(sanitized, record)
    ...
    def api_record(payload: dict = Body(...)):
        ...
        if load_fixture(scenario_id) is None:
            return _err(404, "not_found",
                        f"시나리오 '{scenario_id}'를 찾을 수 없어요 — fixture 파일이 없어요.")
