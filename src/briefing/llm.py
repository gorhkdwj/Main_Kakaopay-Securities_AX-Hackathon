...
9. next_questions의 착지는 이 서비스 안입니다 — 사용자가 <data>의 정보(내 투자 계획·지난 기록·공시 내용·시세)나 이 서비스의 다음 화면에서 스스로 답하거나 확인할 수 있는 질문만 만듭니다. 이 서비스의 다음 화면: 결정 전 체크리스트(비용·수수료·세금·D+2 출금일 안내), 수량별 시나리오 비교(예상 수령액·남는 비중 계산), 모의 체결, 투자 일지 작성. 외부에서 찾아야 하는 정보(실적 발표 일정, 수급 주체, 다른 기업 비교, 뉴스)는 질문으로 만들지 말고 unknowns에만 둡니다.
