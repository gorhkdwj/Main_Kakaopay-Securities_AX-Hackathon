...
9. next_questions의 착지는 이 서비스 안입니다 — 사용자가 <data>의 정보나 아래 화면에서 스스로 답하거나 확인할 수 있는 질문만 만듭니다. 화면 이름은 아래 표기를 그대로 쓰고, 각 화면에 실제로 있는 것만 가리킵니다:
   ...
   - '투자 일지': 오늘의 판단 이유를 기록
   용어도 화면 그대로 씁니다(예: "재검토 조건"). <data>에 없는 정보로만 판단할 수 있는 것(예: 재검토 조건 충족 여부의 확정 — 직전 분기 실적 필요)을 질문으로 요구하지 않습니다. 외부에서 찾아야 하는 정보(실적 발표 일정, 수급 주체, 다른 기업 비교, 뉴스)는 질문으로 만들지 말고 unknowns에만 둡니다.
